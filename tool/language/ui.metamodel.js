...
ui.setupMetamodelUI = function () {
	...
	if (istar.metamodel.nodes.Hazard) {
		istar.metamodel.nodes.Hazard.buttonStatusText = 'Adding <b>Hazard</b>: Click on an actor/role/agent to add a Hazard';
	}
	if (istar.metamodel.nodes.SafetyGoal) {
		istar.metamodel.nodes.SafetyGoal.buttonStatusText = 'Adding <b>SafetyGoal</b>: Click on an actor/role/agent to add a SafetyGoal';
	}
	if (istar.metamodel.nodes.SafetyTask) {
		istar.metamodel.nodes.SafetyTask.buttonStatusText = 'Adding <b>SafetyTask</b>: Click on an actor/role/agent to add a SafetyTask';
	}
	if (istar.metamodel.nodes.SafetyResource) {
		istar.metamodel.nodes.SafetyResource.buttonStatusText = 'Adding <b>SafetyResource</b>: Click on an actor/role/agent to add a SafetyResource';
	}
    ...
    if (istar.metamodel.nodeLinks.ContributionLink) {
	 ...
	 if (istar.metamodel.nodeLinks.ObstructLink) {
        ...
        istar.metamodel.nodeLinks.ObstructLink.buttonStatusText = [
			'',
            'Adding <b>ObstructLink</b> link: click first on an Hazard and then on the SafetyGoal that is obstructed'];
    }
}
