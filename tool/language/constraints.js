...
istar.metamodel.nodeLinks.AndRefinementLink.isValid = function (source, target) {
    ...
    var result = {};
    var isValid = true;
    if ( !(source.isTask() || source.isGoal() || source.isSafetyGoal() || source.isHazard() || source.isSafetyTask()) ) {
        isValid = false;
        result.message = 'the source of an AND-refinement link must be a Goal, or a SafetyGoal, or a Hazard, or a Task or a SafetyTask';
    }
	if ( isValid && (source.isTask() || source.isGoal()) && (target.isSafetyGoal() || target.isHazard() || target.isSafetyTask()) ) {
		isValid = false
		result.message = 'the source of an AND-refinement link in a SafetyGoal must be a SafetyGoal, or a Hazard, or a SafetyTask'
	}
	if ( isValid && (source.isSafetyGoal() && (target.isTask() || target.isHazard() || target.isSafetyTask())) ){
		isValid = false
    ...
    if ( isValid && !(target.isTask() || target.isGoal() || target.isSafetyGoal() || target.isHazard() || target.isSafetyTask()) ) {
        isValid = false;
        result.message = 'the target of an AND-refinement link must be a Goal, or a SafetyGoal, or a Hazard, or a Task or a SafetyTask';
    }
    if ( isValid && (source === target) ) {
        isValid = false;
...
istar.metamodel.nodeLinks.OrRefinementLink.isValid = function (source, target) {
    ...
    if ( !(source.isTask() || source.isGoal() || source.isSafetyGoal() || source.isHazard() || source.isSafetyTask()) ) {
        isValid = false;
        result.message = 'the source of an OR-refinement link must be a Goal, a SafetyGoal, or a Hazard, or a Task, or a SafetyTask';
    }
	if ( isValid && (source.isTask() || source.isGoal()) && (target.isSafetyGoal() || target.isSafetyTask() || target.isHazard()) ) {
		isValid = false
		result.message = 'the source of an OR-refinement link in a SafetyGoal must be a SafetyGoal'
	}
	if ( isValid && (source.isSafetyGoal() && (target.isTask() || target.isHazard() || target.isSafetyTask())) ){
		isValid = false
    ...
    if ( isValid && !(target.isTask() || target.isGoal() || target.isSafetyGoal() || target.isSafetyTask() || target.isHazard()) ) {
        isValid = false;
        result.message = 'the target of an OR-refinement link must be a Goal, a SafetyGoal, or a Hazard, or a SafetyTask, or a Task';
    }
    if ( isValid && (source === target) ) {
        isValid = false;
...
istar.metamodel.nodeLinks.ObstructLink.isValid = function (source, target) {
    'use strict';
    ...
    if ( !source.isHazard()) {
        isValid = false;
        result.message = 'the source of an Obstruct-refinement link must be a Hazard';
    }
    ...
    if ( isValid && (source.isDependum() || target.isDependum()) ) {
        isValid = false;
        result.message = 'you cannot make an Obstruct-refinement link with a dependum';
    }
    if ( isValid && (source.attributes.parent !== target.attributes.parent) ) {
        isValid = false;
        result.message = 'the source and target of an Obstruct-refinement link must pertain to the same actor';
    }
    if ( isValid && istar.isThereLinkBetween(source, target)) {
        isValid = false;
        result.message = 'there can only be one refinement link between the same two elements';
